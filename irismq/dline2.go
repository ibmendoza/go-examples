...
//TODO: this is not goroutine-safe
...
func main() {
	...
	*/
	...
	//separate function
	log.Println(ancestry("51"))
}

func iifInt(condition bool, intIfTrue, intIfFalse int) int {
	if condition {
		return intIfTrue
	} else {
		return intIfFalse
...
func getIDUpline(id string) int {
	var idupline int
	sql := "select idindirect from code where iddownline = " + id

	dbx.Context().Begin().Get(&idupline, sql)

	return iifInt(idupline > 0, idupline, 0)
}

//TODO: This is a prime example for context within HTTP handler middleware
//https://joeshaw.org/revisiting-context-and-http-handler-for-go-17/
//used to compute payout
func ancestry(id string) []int {
	var slc []int
	var idUpline int

	for {
		idUpline = getIDUpline(id)
		log.Println(idUpline)
		if idUpline > 0 {
			if idUpline == 1 {
				slc = append(slc, 1)
				break
			} else {
				slc = append(slc, idUpline)
				id = strconv.Itoa(idUpline)
			}
		} else {
			break
		}
